...
def test_shared_map_covers_openms_and_diann_reagents():
...
def test_new_plexes_have_contiguous_one_based_ids(plex):
...
def test_normalize_label(raw, canonical):
    assert cm.normalize_label(raw) == canonical


def test_channel_labels_is_inverse_of_channel_map():
...
def test_labels_for_plex_is_index_ordered():
...
def test_openms_reexports_shared_map():
    assert openms_constants.CHANNEL_MAP is cm.CHANNEL_MAP


def test_diann_plex_membership_is_sourced_from_shared_map():
...
def test_plexdia_detects_label_free_via_synonym():
