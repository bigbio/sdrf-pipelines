...
class DataAnalysisParams:
    def __init__(self) -> None:
        """Add data analysis parameters from yaml file to sdrf file."""
        self.warnings = {}

    def add_analysis_params(self):
        ...
        for p in mapping:
            ...
            self.check_values(p, pname, ptype, pvalue)
            ...
            if pname == "fragment_mass_tolerance" or pname == "precursor_mass_tolerance":
                ...
                if unit != "Da" and unit != "ppm":
                    exit(
                        "ERROR: "
                        + pname
                        + ' allows only units of "Da" and "ppm", separated by space from the value!!\nWe found '
                        + unit
                    )
            ...
            ## ENZYME AND MODIFICATIONS: LOOK UP ONTOLOGY VALUES
                ...
                if ols_out == None or len(ols_out) > 1:
                    exit(
                        "ERROR: enzyme "
                        + pvalue
                        + " not found in the MS ontology, see https://bioportal.bioontology.org/ontologies/MS/?p=classes&conceptid=http%3A%2F%2Fpurl.obolibrary.org%2Fobo%2FMS_1001045 for available terms"
                    )
        ...
        for m in mods:
            ...
            if len(tmod) < 2:
                exit(
                    ...
                    + m
                    + ". It should be PSI_MS_NAME of RESIDUE. Note that it should be single residues"
                )
            modname = tmod[0]
            modpos = tmod[1]
            ...
            if found == []:
                exit(
                    "ERROR: "
                    + m
                    + ' not found in Unimod. Check the "PSI-MS Names" in unimod.org. Also check whether you used space between the comma separated modifications'
                )
            modtype = "fixed"
            ...
            if re.fullmatch("[A-Z]", modpos):
                ...
                exit(
                    ...
                    + modpos
                    + '. Should be either one upper case letter or any of "Protein N-term", "Protein C-term", "Any N-term", "Any C-term"'
                )
        return mod_columns
...
def main():
    DataAnalysisParams().add_analysis_params()
