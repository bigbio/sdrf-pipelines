...
class OpenMS:
  ...
  def openms_convert(self, sdrf_file: str = None, keep_raw: bool = False, onetable: bool = False, legacy: bool = False,
                     verbose: bool = False):
    ...
    for index, row in sdrf.iterrows():  # does only work for label-free not for multiplexed. TODO
      ...
      raw = row["comment[data file]"]
      f.write(URI + "\t" + raw + "\t" + file2mods[raw][0] + "\t" + file2mods[raw][1] + "\t" + file2label[raw] + "\t" +
              file2pctol[
                raw] + "\t" + file2pctolunit[raw] + "\t" + file2fragtol[raw] + "\t" + file2fragtolunit[raw] + "\t" +
              file2diss[
                raw] + "\t" + file2enzyme[raw] + "\n")
    f.close()
